...
chmod +x BackEnd/deployment/build.sh
chmod +x BackEnd/deployment/start.sh
chmod +x BackEnd/deployment/start_simple.sh
...
if [ ! -f "BackEnd/deployment/render.yaml" ]; then
    echo -e "${RED}Error: render.yaml file not found in BackEnd/deployment directory!${NC}"
    exit 1
fi
...
# Verify Dockerfile
if [ ! -f "BackEnd/deployment/Dockerfile" ]; then
    echo -e "${RED}Error: BackEnd/deployment/Dockerfile not found!${NC}"
    exit 1
fi
